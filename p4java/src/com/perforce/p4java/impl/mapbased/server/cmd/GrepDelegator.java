...
public class GrepDelegator extends BaseDelegator implements IGrepDelegator {
    ...
    public List<IFileLineMatch> getMatchingLines(@Jdk7Nonnull final List<IFileSpec> fileSpecs,
            @Jdk7Nonnull final String pattern, @Jdk7Nullable final List<IServerMessage> infoLines,
        ...
        if (nonNull(resultMaps)) {
            for (Map<String, Object> map : resultMaps) {
                IServerMessage message = ResultMapParser.getErrorStr(map);
                throwRequestExceptionOnError(message);
