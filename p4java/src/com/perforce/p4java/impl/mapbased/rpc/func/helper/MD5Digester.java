...
import com.perforce.p4java.util.compat.BOMInputStream;
import com.perforce.p4java.util.compat.ByteOrderMark;
...
import com.perforce.p4java.util.compat.StringUtils;
...
public class MD5Digester {
	...
	public MD5Digester(@Jdk7Nonnull int bufferSize) {
		this();
		this.bufferSize = Validate.notNull(bufferSize);
	}
	 ...
	 */
	@Jdk7Nullable
	public String digestFileAs32ByteHex(@Jdk7Nonnull File file) {
		Validate.notNull(file, "Null file passed to MD5Digester.digestFileAs32ByteHex()");
		if (NioFiles.isReadable(file)) {
	 ...
	 */
	@Jdk7Nullable
	public String digestFileAs32ByteHex(@Jdk7Nonnull File file, @Jdk7Nullable Charset charset) {
	...
	@Jdk7Nullable
	public String digestFileAs32ByteHex(@Jdk7Nonnull File file, @Jdk7Nullable Charset charset,
	                                    boolean isRequireLineEndingConvert, @Jdk7Nullable ClientLineEnding
            clientLineEnding) {
	...
	private void digestEncodedStreamToUtf8(@Jdk7Nonnull InputStream inStream, @Jdk7Nonnull Charset charset,
	                                       boolean isRequireLineEndingConvert, @Jdk7Nullable ClientLineEnding
            clientLineEnding)
			throws IOException {

        BOMInputStream unicodeInputStream = null;
		try {
            unicodeInputStream = new BOMInputStream(inStream, false,
	...
	private ByteBuffer findAndReplaceEncodedClientLineEndingIfRequireLineEndingCovert(
			@Jdk7Nonnull InputStreamReader encodedStreamReader,
			@Jdk7Nonnull CharsetEncoder utf8CharsetEncoder, @Jdk7Nonnull ByteBuffer initialUtf8ByteBuffer,
			@Jdk7Nullable ClientLineEnding clientLineEnding) throws IOException {
	...
	private int findOffsetOfNextClientLineEndingIfReadBytesEndWithFirstByteOfClientLineEnding(
			byte lastByte, @Jdk7Nullable ClientLineEnding clientLineEnding) {
	...
	private boolean isRequireConvertClientOrLocalLineEndingToServerFormat(
			@Jdk7Nullable ClientLineEnding clientLineEnding) {
	...
	private ByteBuffer convertToP4dServerEndingsIfRequired(@Jdk7Nonnull byte[] sourceBytes,
	                                                       final int start, final int length, @Jdk7Nullable
	...
	private boolean doesSourceBytesUseSameClientLineEnding(@Jdk7Nonnull byte[] sourceBytes,
	...
	private void digestStream(@Jdk7Nonnull InputStream inStream, boolean isRequireLineEndingConvert,
	                          @Jdk7Nullable ClientLineEnding clientLineEnding) throws IOException {
	...
	private ByteBuffer findAndReplaceNonEncodedClientLineEndingIfRequireLineEndingConvert(
			@Jdk7Nonnull InputStream inStream, @Jdk7Nonnull final byte[] readBuffer,
			final int totalBytesReadIntoBuffer, @Jdk7Nullable ClientLineEnding clientLineEnding)
			throws IOException {
