...
public class RpcStreamConnection extends RpcConnection {
    ...
    public RpcPacket getRpcPacket(final RpcPacketFieldRule fieldRule,
        ...
        // } catch (Throwable thr) {
            ...
            Log.error("Unexpected exception: %s", thr.getLocalizedMessage());
            Log.exception(thr);
            // p4ic4idea: remove useless getMessage() call
            throwP4JavaError(thr);
        }
     ...
     */
    private int continueReadIfGetPartialRead(@Jdk7Nonnull final byte[] preambleBytes,
            final int bytesRead, @Jdk7Nonnull final AtomicLong streamRecvs)
     ...
     * wrong...
     */
    private int continueReadIfIncompleteRead(@Jdk7Nonnull final AtomicLong streamRecvs,
            final int payloadLength, @Jdk7Nonnull final byte[] packetBytes, final int packetBytesRead)
            throws IOException, ConnectionException {
    ...
    public long putRpcPackets(@Jdk7Nonnull RpcPacket[] packets) throws ConnectionException {
     ...
     * </pre>
     */
    public long putRpcPacket(@Jdk7Nonnull RpcPacket packet) throws ConnectionException {
    ...
    private void processNameArgs(@Jdk7Nonnull RpcPacket packet,
            @Jdk7Nonnull final RpcPacketSupplier argsSupplier) {
    ...
    private void reallocateSendBufferInPutPacketIfRunOut(@Jdk7Nonnull final RpcPacketSupplier supplier,
            @Jdk7Nonnull final byte[] fieldBytes, final int reallocateIncrement) {
    ...
    private void processStringArgs(@Jdk7Nonnull RpcPacket packet,
            @Jdk7Nonnull final RpcPacketSupplier argsSupplier) {
    ...
    private void processExternalEnv(@Jdk7Nonnull RpcPacket packet,
            @Jdk7Nonnull final RpcPacketSupplier argsSupplier) {
    ...
    private void processFuncName(@Jdk7Nonnull RpcPacket packet, @Jdk7Nonnull RpcPacketSupplier supplier) {
    ...
    private void calculatePreambleBytesAndSendtoDownstream(
            @Jdk7Nonnull final RpcPacketSupplier argsSupplier) throws ConnectionException {
