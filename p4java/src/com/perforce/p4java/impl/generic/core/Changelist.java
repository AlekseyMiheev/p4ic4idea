...
public class Changelist extends ChangelistSummary implements IChangelist {
	...
	public List<IFileSpec> getFiles(boolean refresh, boolean bypassServer)
		...
		if (this.serverImpl == null) {
			// p4ic4idea: change exception, as this is an API error.
	...
	public InputStream getDiffs(DiffType diffType)
		...
		if (this.serverImpl == null) {
            // p4ic4idea: change exception, as this is an API error.
			throw new IllegalStateException("Changelist not associated with a Perforce server");
		}
		...
		if (retMaps != null) {
			for (Map<String, Object> map : retMaps) {
				if (map.get("submittedChange") != null) {
					...
					// p4ic4idea: this isn't necessary as an info message, and indeed
                    // doesn't add anything to the result.  Removed.
	...
	public void refresh()
		...
		if (cList == null) {
		    // TODO is this the right exception to throw?  Doesn't seem like it.
            // p4ic4idea: change exception, as this is an API error.
            throw new IllegalStateException("Changelist is null when refreshing from the Perforce server");
		}
	...
	public void update(Options opts) throws ConnectionException, RequestException, AccessException {
		...
		if ((id == IChangelist.UNKNOWN) || (id == IChangelist.DEFAULT)) {
            // p4ic4idea: change exception, as this is an API error.
            throw new IllegalArgumentException("Tried to update new or default changelist");
		}
		if (opts != null) {
			if (!(opts instanceof ChangelistOptions)) {
                // p4ic4idea: change exception, as this is an API error.
                throw new IllegalArgumentException("Options parameter is not an instanceof ChangelistOptions");
			}
		}
